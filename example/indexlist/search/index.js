...
Page({
    ...
    onLoad() {
        this.setData({ items: ITEMS, autoSearch: this.autoSearch.bind(this) });
    },
    ...
    autoSearch: function (value) {
        ...
        this.fitterItems(value);
    },
    fitterItems(value) {
        const { items } = this.data;
        const arr = items.filter((item) => item.key === value || item.name.indexOf(value) > -1);
        this.setData({
            items: arr
        });
    }
