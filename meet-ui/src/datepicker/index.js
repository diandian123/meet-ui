    ...
    type(value) {
      ...
      if (value === "time") {
        ...
        selectedText = defaultValue
          ? `${nowTime.getFullYear()}年${nowMonth + 1}月`
          : "";
        ...
        // 传入默认值则field回显
        selectedText = defaultValue
          ? `${nowTime.getFullYear()}-${SelectMonth}-${SelectDay}`
          : "";
        // 设置初始index
  ...
  methods: {
    ...
    getPicker() {
      if (!this.picker) {
        ...
        picker.setColumnItems = (...args) =>
          setColumnItems.apply(picker, [...args, false]);
      }
      return this.picker;
    },
    // 获取时分数据
    ...
    judgeDate(columnIndex, lastSelectValue, value, selectDay) {
      ...
      if (
        columnIndex === 0 &&
        this.judgeLeapYear(lastSelectValue) !== this.judgeLeapYear(value)
      ) {
        ...
        if (
          longMonth.indexOf(lastSelectValue) > -1 &&
          longMonth.indexOf(value) > -1
        ) {
          console.log("不需要更新");
          return;
        } else if (
          shortMonth.indexOf(lastSelectValue) > -1 &&
          shortMonth.indexOf(value) > -1
        ) {
    ...
    formatDate(value, type) {
      ...
      if (type === "datetime") {
        ...
        formatValue = `${selectedText
          .slice(0, 3)
          .join("-")} ${selectedText.slice(3, 5).join(":")}`;
      } else if (type === "time") {
        // 时间显示
